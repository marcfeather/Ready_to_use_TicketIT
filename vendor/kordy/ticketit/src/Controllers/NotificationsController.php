...
class NotificationsController extends Controller
{
    public function newComment(Comment $comment)
    {
        ...
        $this->sendNotification($template, $data, $ticket, $notification_owner,
            trans('ticketit::lang.main-notify-text').trans('ticketit::lang.notify-new-comment-from').$ticket->id, 'comment');
    }
    ...
    public function ticketStatusUpdated(Ticket $ticket, Ticket $original_ticket)
    {
        ...
        if (strtotime($ticket->completed_at)) {
            $this->sendNotification($template, $data, $ticket, $notification_owner,
                trans('ticketit::lang.main-notify-text').trans('ticketit::lang.notify-updated').$ticket->id.trans('ticketit::lang.notify-status-to-complete'), 'status');
        } else {
            $this->sendNotification($template, $data, $ticket, $notification_owner,
                trans('ticketit::lang.main-notify-text').trans('ticketit::lang.notify-updated').$ticket->id.trans('ticketit::lang.notify-status-to').$ticket->status->name, 'status');
        }
    }
    ...
    public function ticketAgentUpdated(Ticket $ticket, Ticket $original_ticket)
    {
        ...
        $this->sendNotification($template, $data, $ticket, $notification_owner,
            trans('ticketit::lang.main-notify-text').trans('ticketit::lang.notify-transferred').$ticket->id, 'agent');
    }
    ...
    public function newTicketNotifyAgent(Ticket $ticket)
    {
        ...
        $this->sendNotification($template, $data, $ticket, $notification_owner,
           trans('ticketit::lang.main-notify-text').trans('ticketit::lang.notify-created-ticket').$ticket->id, 'agent');
    }
